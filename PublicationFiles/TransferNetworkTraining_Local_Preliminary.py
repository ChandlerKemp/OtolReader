...
training_path = '../../TrainingSampleDatabase/'
...
def make_fold_train_array(train_X, train_y, foldinds):
    tr = []
    val = []
    tr_labs = []
    val_labs = []
...
def classification_model_trainer(training_array_path, n_cvfolds, n_img, cv_order):
    ...
    for cvind in range(n_cvfolds):
        ...
        tr, tr_labs, val, val_labs = make_fold_train_array(train_X, train_y, foldinds)
        print('tr shape:', tr.shape)
        model = classification_model_init(tr)
        model.fit(tr, tr_labs, epochs=10, verbose=0, validation_data=(val, val_labs))
        #
        # # save the model
